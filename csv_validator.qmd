...
<script>
  ...
  const VALIDATION_RULES = {
    ...
    if (rules?.required) {
      #' Checks if the given condition is true and determines if the function is required.
      #'
      #' This function evaluates a condition to determine whether it is necessary to proceed
      #' with the execution of the function. It is typically used to ensure that certain
      #' prerequisites or logical checks are met before performing further operations.
      #'
      #' @param condition A logical expression or value that determines whether the function is required.
      #'                  If `TRUE`, the function is considered required; otherwise, it is not.
      #' @return A logical value indicating whether the function is required (`TRUE`) or not (`FALSE`).
      #' @examples
      #' # Example usage:
      #' is_required <- required(condition = x > 5)
      #' if (is_required) {
  ...
  function validateRow(row) {
    return Object.keys(row).flatMap(fieldName => validateField(fieldName, row[fieldName]));
  }
